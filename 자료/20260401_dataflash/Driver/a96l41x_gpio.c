...
void Port_Initial(void)
{
		...
		P0FSRH  = 0
		| ( 0 << 6)       //P07     // 0 : I/O (EINT3), 1 : ICS1
		| ( 0 << 4)       //P06     // 0 : I/O (EINT2), 1 : ICS0
		| ( 0 << 2)       //P05     // 0 : I/O, 1 : OP0P,    
		| ( 0 << 0);      //P04     // 0 : I/O, 1 : OP0N
		
		P0FSRL  = 0
		| ( 0 << 6)       //P03     // 0 : I/O, 1 : OP0OUT,   2 : AN3,      3 : Not used
		| ( 0 << 4)       //P02     // 0 : I/O, 1 : OP1P,   2 : AN2,      3 : Not used
		| ( 0 << 2)       //P01     // 0 : I/O (EINT1),	1 : OP1N,  2 : AN1,	3 : Not used
		| ( 0 << 0);      //P00     // 0 : I/O (EINT0),	1 : OP1OUT,   2 : AN0,	3 : Not used
		...
		P1IO	= _0011_1111;		// 0 : Input,	1 : Output
		
		P1FSRH  = 0
		| ( 0 << 2)       //P15  // 0 : I/O,	1 : LDO23,   2 : AN6,	3 : (SDA)
		| ( 0 << 0);      //P14  // 0 : I/O (EINT11),	1 : T1O/PWM1O,   2 : SCL,	3 : Not used
		
		P1FSRL  = 0
		| ( 0 << 6)       //P13  // 0 : I/O (EC1),	1 : (RXD/MISO)
		| ( 0 << 4)       //P12  // 0 : I/O (EC0),	1 : (TXD/MOSI)
		| ( 0 << 2)       //P11  // 0 : I/O (EINT10),	1 : T0O/PWM0O
		| ( 0 << 0);      //P10  // 0 : I/O, 	1 : (SCK)
		...
		P2FSR   = 0
		| ( 0 << 6) 		// 0 : P23,  1 : TXD/MOSI,	2 : AN5,     3 : Not used
		| ( 0 << 4) 		// 0 : P22,  1 : RXD/MISO,	2 : AN4,     3 : Not used
		| ( 0 << 2)         // 0 : P21 (EC2),  1 : SCK,   2 : (SCL),     3 : Not used
		| ( 0 << 0);        // 0 : P20 (EINT12),  1 : T2O/PWM2O,   2 : SDA,     3 : Not used
...
void Port_ConfigureInterrupt(uint8_t channel, uint8_t edge)
{
	uint8_t temp_reg;
		...
		switch(channel)
		{
			case 0:
				temp_reg = EIPOL0;
				temp_reg &= ~(0x3<<(channel<<0));
				temp_reg |= (edge<<(channel<<0));
				
				EIPOL0 = temp_reg;
			case 1:
				temp_reg = EIPOL0;
				temp_reg &= ~(0x3<<(channel<<2));
				temp_reg |= (edge<<(channel<<2));
				
				EIPOL0 = temp_reg;
			case 2:
				temp_reg = EIPOL0;
				temp_reg &= ~(0x3<<(channel<<4));
				temp_reg |= (edge<<(channel<<4));
				
				EIPOL0 = temp_reg;
			case 3:
				temp_reg = EIPOL0;
				temp_reg &= ~(0x3<<(channel<<6));
				temp_reg |= (edge<<(channel<<6));
